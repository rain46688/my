...
body {
	...
	background-image: url('${path}/resources/images/chatRoom.png');
			...
			success : function(data) {
				...
				$.each(data,function(i,msg){
					if(msg["msg"] == "SYS1"){
						...
						time='오'+arr[1];
						time = time.substring(0,time.lastIndexOf(':'));
						html+="<div class='tmp'><img class='profile' src='{path}/resources/images/"+msg["chatProfile"]+"'><div class='nick'>"+msg["sendNickName"]+"</div><div class='othermsg'>"+msg["msg"]+"</div><div class='date2'>"+time+"</div></div>";
					}
				});
				$("#ChatArea").html(html);
...
var socket=new WebSocket("ws://localhost:9090${path}/socket");   --%>
...
/*   socket.onopen = function(e) {
	...
	let day = _today.format('yyyy-MM-dd a/p hh:mm:ss');
		console.log("day : "+day); */
		socket.send(JSON.stringify(new Message("${boardId}","${m.nickname}","SYS1","","")));
	...
	socket.onmessage = function(e) {
		...
		console.log("메세지 보낸 사람 닉네임 : "+msg["chat_writer_nickname"]);
		console.log("현재 세션 사람 닉네임 : "+"${m.nickname}");
		
		//입&퇴장시 메세지 일반 메세지 출력 분기처리 
		if(msg["chat_content"] == "SYS1"){
			let html ="";
			//관리자일 경우 일반 유저일 경우 분기처리 
			 if(msg["chat_writer_nickname"] == "ADMIN"){
					html="<div class='tmp'><div class='admin'> ADMIN 관리자가 접속하였습니다." + "</div></div>";
			}else{ 
				html="<div class='tmp'><div class='conn'>"+msg["chat_writer_nickname"]+" 이(가) 합류하였습니다." + "</div></div>";
			}
			$("#ChatArea").html($("#ChatArea").html()+html); 
		}else if(msg["chat_content"] == "SYS2"){
			html="<div class='tmp'><div class='conn'>"+msg["chat_writer_nickname"]+" 이(가) 퇴장하였습니다." + "</div></div>";
			$("#ChatArea").html($("#ChatArea").html()+html); 
			//관리자인 경우 분기 처리
		}else if(msg["chat_writer_nickname"] == "ADMIN"){
			let html="<div class='tmp'><div class='admin'> 시스템 관리자 : "+msg["chat_content"] + "</div></div>";
			$("#ChatArea").html($("#ChatArea").html()+html); 
		}else if(msg["chat_writer_nickname"] == "${m.nickname}"){
			//자기 메세지일 경우 분기처리
				let time = msg["chat_time"];
				let arr = time.split('오');
				time='오'+arr[1];
				time = time.substring(0,time.lastIndexOf(':'));
	 		let html="<div class='tmp'><div class='mymsg'>"+msg["chat_content"]+"</div><div class='date'>"+time+"</div></div>";
			$("#ChatArea").html($("#ChatArea").html()+html); 
		}else{
			//상대방 메세지일 경우 분기처리
				let time = msg["chat_time"];
				...
				time='오'+arr[1];
				time = time.substring(0,time.lastIndexOf(':'));
				console.log("msg['chat_profile_image']"+msg["chat_profile_image"]);
			let html="<div class='tmp'><img class='profile' src='${path}/resources/images/"+msg["chat_profile_image"]+"'><div class='nick'>"+msg["chat_writer_nickname"]+"</div><div class='othermsg'>"+msg["chat_content"]+"</div><div class='date2'>"+time+"</div></div>";
			$("#ChatArea").html($("#ChatArea").html()+html);
		}
	...
	function sendMessage() {
		...
		socket.send(JSON.stringify(new Message("${boardId}",user,txt.val(),"${memberPicture}",day)));
	...
	//메세지 객체
	function Message(chat_board_id, chat_writer_nickname, chat_content, chat_profile_image, chat_time){
		this.chat_writer_nickname=chat_writer_nickname;
		this.chat_content=chat_content;
		this.chat_board_id = chat_board_id;
